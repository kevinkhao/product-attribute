...
class TestProductProfile(TransactionCase):
    ...
    def _setup_profile_data(self):
        self.profile_own = self.env.ref("product_profile_example.own")
        ...
        self.profile_complete = self.env.ref("product_profile_example.complete_prof")
        ...
        self.profile_manuf = self.env.ref('product_profile_example.manuf_prof')
    ...
    def test_on_set_tmpl_profile(self):
        ...
        for key in self.profile_own_nondefaults:
            self.assertEqual(self.profile_own_nondefaults[key], getattr(self.analysis_tmpl, key))
        for key in self.profile_own_defaults:
            self.assertEqual(self.profile_own_defaults[key], getattr(self.analysis_tmpl, key))
        ...
        for key in self.profile_complete_nondefaults:
            self.assertEqual(self.profile_complete_nondefaults[key], getattr(self.analysis_tmpl, key))
        for key in self.profile_own_defaults:
    ...
    def test_on_set_prd_profile(self):
        ...
        for key in self.profile_own_nondefaults:
            self.assertEqual(self.profile_own_nondefaults[key], getattr(self.analysis_prd, key))
        for key in self.profile_own_defaults:
            self.assertEqual(self.profile_own_defaults[key], getattr(self.analysis_prd, key))
    ...
    def test_on_write_profile_defaults(self):
        ...
        self.profile_manuf.write({"profile_default_categ_id": False})
        self.assertNotEqual(product.categ_id, False)
